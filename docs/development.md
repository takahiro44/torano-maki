...
`torano-maki` をローカルで動かし、開発するための手順。
プロダクトの概要は [README](../README.md) を参照。

> このドキュメントは、インターンシップ期間中に4人で開発を進めるために書かれたもの。
> 当時の内容をそのまま残している。
...
**NVIDIA DGX Spark**（貸し出し機）

LLM推論（vLLM）と音声認識（faster-whisper）を載せ、OpenAI互換APIをLANで共有した。
実測値と判明した制約は [`setup-notes.md`](setup-notes.md) を参照。

ARM64 + CUDA のため、x86_64向けのビルド済みパッケージが利用できない場合がある。
環境構築でハマった内容は [`docs/setup-notes.md`](setup-notes.md) に記録すること。
...
解決したら [`docs/setup-notes.md`](setup-notes.md) に記録すること。
