...
終わると振り返りが出る。**できていない点はできていないと返す。**

![ロープレの振り返り](docs/images/roleplay-feedback.png)

下部の「参照した社内事例」は、**この練習で実際に使われたナレッジをサーバ側の記録から並べたもので、
LLMに列挙させていない。** 出典は発話単位まで辿れ、話者と時刻が付く。
生成した文章の中に出典を書かせると、それらしい引用を作られても検証できないため
（[`docs/decisions.md`](docs/decisions.md) 2026-08-26）。
